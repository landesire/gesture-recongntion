...
def data_prepare():
    ...
    with tf.device('/gpu:0'):
        ...
        rebuild_feature_data=np.zeros((data.shape[0],6600))
        ...
        for index in range(len(data)):
            ...
            rebuild_feature_data[index] = curret_feature
            # print("after change")
            # print(data[index][150])
            curret_feature.clear()
        print(rebuild_feature_data)
        print(rebuild_feature_data[0])
        print(len(rebuild_feature_data[0]))

        # 生成标注类别1-10类
        for i in range(len(rebuild_feature_data)):
            label = (i / 800)
        ...
        # print y_train2
        print(len(y_label))
        all_data = np.hstack((y_label, rebuild_feature_data))
...
def LSTM(train_feature,train_label,test_feature,test_label):
    ...
    with tf.device('/gpu:0'):
        ...
        # Define weights
        weights = {
            'out': tf.Variable(tf.random_normal([num_hidden, num_classes], mean=0.0,
                                                stddev=math.sqrt(2.0 / (num_input + num_classes))))
        }
        biases = {
